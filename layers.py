...
class Sigmoid:
    def forward(self, _input):
        return [1 / (1 + np.exp(-x)) for x in _input]
...
class Softmax:
    def forward(self, _input):
        return [np.exp(x - np.max(x)) / np.sum(np.exp(x - np.max(x))) for x in _input]
...
class Dense:
    ...
    def forward(self, _input):
        return [np.add(wa, self.bias) for wa in np.matmul(_input, self.weights)]
